...
**5. 본문 문장에 긴 줄표(—)를 쓰지 않는다.** 한국어 문장에서 줄표로 구절을 잇지 말고 문장을 끊어 씁니다. ("주소를 잘 간수하는 것으로는 부족합니다 — 실수 한 번이면 끝이니까요." → 줄표 뒤를 삭제하거나 별도 문장으로.) 섹션 제목·카드 라벨의 구분 용도(예: "실습 — 배포 4단계")는 예외입니다.

**6. 자명한 문장은 넣지 않는다.** "폴더를 나눠 두면 파일이 섞이지 않습니다" 같은 문장은 지면만 차지합니다. 수강생이 모를 만한 것, 틀리기 쉬운 것만 씁니다.
